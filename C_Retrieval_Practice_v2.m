...
for y= 1:numTrials
%  x=(n); %from the encoding list, pick up the element selcted...
%  y=rows_rand(n); %and put it in this randomized position

 stimuli_list{1, 1}{y, 1} = inputfile{1, 1}{y, 1};
 stimuli_list{1, 2}{y, 1} = inputfile{1, 2}{y, 1};  
 stimuli_list{1, 3}{y, 1} = inputfile{1, 4}{y, 1};  
 stimuli_list{1, 4}{y, 1} = inputfile{1, 5}{y, 1};  
 stimuli_list{1, 5}{y, 1} = inputfile{1, 6}{y, 1}; % trialt ype
 stimuli_list{1, 6}{y, 1} = inputfile{1, 7}{y, 1}; % cue type
end
...
% try
    ...
    for i = 1:numTrials
        ...
        t_fixation_offset1(i)=Screen('Flip',windowPtr,t_fixation_onset1(i)+fixation_duration-slack);        
        % ---------------- Cue ---------------- %
    ...
    t_fixation_offset2(i)=Screen('Flip',windowPtr,t_fixation_onset2(i)+fixation_duration-slack);
    % ---------------- Selection ---------------- %
