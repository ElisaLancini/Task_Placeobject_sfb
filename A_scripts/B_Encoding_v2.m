...
% presented twice, once in the first half of the experiment, once in
% the secon half of the experiment.

% The room pictures is the same but
% what changes is the position of the cue (e.g once the cue is on the
% right, once is on the left)

% each picture is always followed by either a neutral or a negative picture

% the picture that follow the presentation will ALWAYS have the same
% emotional valence.
...
for i = 1:numTrials*2
    ...
    t_fixation_offset3(i)=Screen('Flip',windowPtr,t_fixation_onset3(i)+fixation_duration(i,3)-slack);
    % ---------------- Feedback ------ %
    Screen('DrawTexture', windowPtr, pic_room_texture, [], topcentral);
